...
def get_json(url: str, api_key: str, timeout_seconds: float = 10) -> dict[str, Any]:
    request = Request(url, headers={"Authorization": f"Bearer {api_key}", "Accept": "application/json", "User-Agent": USER_AGENT})
...
def classify_failure(record: dict[str, Any], provider: str, exc: BaseException, auth_help: str) -> dict[str, Any]:
    """Turn an expected probe failure into the panel's documented state."""
    if isinstance(exc, HTTPError):
        if exc.code in (401, 403):
            return endpoint_problem(record, "API key rejected", auth_help)
...
def find_key(env_name: str, config_provider: str) -> str | None:
    ...
    try:
        ...
        candidate = data.get(config_provider, {}).get("apiKey") if isinstance(data, dict) else None
